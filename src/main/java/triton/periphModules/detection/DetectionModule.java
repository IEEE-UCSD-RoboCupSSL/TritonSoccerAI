...
import triton.legacy.oldGrSimProto.protosrcs.MessagesRobocupSslDetection;
...
public class DetectionModule implements Module {
    ...
    // Subscribers
    private final Subscriber<MessagesRobocupSslDetection.SSL_DetectionFrame> visionSub_OldProto;
     ...
     */
    public void update(MessagesRobocupSslDetection.SSL_DetectionFrame frame) {
        if (frame == null) return;
        double time = frame.getTCapture();

        for (MessagesRobocupSslDetection.SSL_DetectionRobot robotFrame : frame.getRobotsBlueList()) {
        ...
        for (MessagesRobocupSslDetection.SSL_DetectionRobot robotFrame : frame.getRobotsYellowList()) {
            int id = robotFrame.getRobotId();
