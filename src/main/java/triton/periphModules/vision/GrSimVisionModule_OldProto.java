...
import triton.legacy.oldGrSimProto.protosrcs.*;
...
public class GrSimVisionModule_OldProto extends VisionModule {
    ...
    private final static int MAX_BUFFER_SIZE = 600000;
    private final Publisher<triton.legacy.oldGrSimProto.protosrcs.MessagesRobocupSslDetection.SSL_DetectionFrame> visionPub;
...
//                    jsonConfig.conn().getGrsimMcPort());
    ...
    private void processGeometry(MessagesRobocupSslGeometry.SSL_GeometryData geometryData) {
        MessagesRobocupSslGeometry.SSL_GeometryFieldSize field = geometryData.getField();

        // Field Lines
        GvcGeometry.FIELD_LINES = new HashMap<>();
        for (MessagesRobocupSslGeometry.SSL_FieldLineSegment sslLine : field.getFieldLinesList()) {
        ...
        // Field Circle
        MessagesRobocupSslGeometry.SSL_FieldCicularArc arc = field.getFieldArcs(0);
